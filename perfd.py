...
os.system("perf record -e cycles:u -j any,u -a -o  " + temp_perf_file.name + "  -- sleep 10")
...
for name in samples_per_obj_file:      
    database.write(perf_name= temp_perf_file.name, name_obj_file= name, amount_of_samples= samples_per_obj_file[name])
